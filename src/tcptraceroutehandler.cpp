...
int CTcpTracerouteHandler::handle_tcp_traceroute()
{
    ...
    if (rHost.empty())
        rHost = get_value_from_string(request, get_lower_string(HTTP_HOST));
    if (rAccessMethod.empty())
        rAccessMethod = get_value_from_string(request, get_lower_string(HTTP_ACCESS_METHOD));
    if (rAccessHeaders.empty())
        rAccessHeaders = get_value_from_string(request, get_lower_string(HTTP_ACCESS_HEADERS));
    if (rOrigin.empty())
        rOrigin = get_value_from_string(request, get_lower_string(HTTP_ORIGIN));
    if (rConnection.empty())
        rConnection = get_value_from_string(request, get_lower_string(HTTP_CONNECTION));
    if (rContentLength.empty())
        rContentLength = get_value_from_string(request, get_lower_string(HTTP_CONTENT_LENGTH));

    print_request(rHost, rAccessMethod, rAccessHeaders, rOrigin, rConnection, rContentLength);

    if (((request.find(HTTP_OPTIONS) != string::npos) ||
         (request.find(get_lower_string(HTTP_OPTIONS)) != string::npos)) &&
        ((request.find(HTTP_HOST) != string::npos) ||
         (request.find(get_lower_string(HTTP_HOST)) != string::npos)) &&
        ((request.find(HTTP_ACCESS_METHOD) != string::npos) ||
         (request.find(get_lower_string(HTTP_ACCESS_METHOD)) != string::npos)) &&
        ((request.find(HTTP_ACCESS_HEADERS) != string::npos) ||
         (request.find(get_lower_string(HTTP_ACCESS_HEADERS)) != string::npos)) &&
        ((request.find(HTTP_ORIGIN) != string::npos) ||
         (request.find(get_lower_string(HTTP_ORIGIN)) != string::npos)) &&
        ((request.find(HTTP_CONNECTION) != string::npos) ||
         (request.find(get_lower_string(HTTP_CONNECTION)) != string::npos)))
    {
        ...
        handle_preflight_request(rAccessMethod, rAccessHeaders, rOrigin, rConnection);
    ...
    else if (((request.find(HTTP_POST) != string::npos) ||
              (request.find(get_lower_string(HTTP_POST)) != string::npos)) &&
             ((request.find(HTTP_HOST) != string::npos) ||
              (request.find(get_lower_string(HTTP_HOST)) != string::npos)) &&
             ((request.find(HTTP_ORIGIN) != string::npos) ||
              (request.find(get_lower_string(HTTP_ORIGIN)) != string::npos)) &&
             ((request.find(HTTP_CONNECTION) != string::npos) ||
              (request.find(get_lower_string(HTTP_CONNECTION)) != string::npos)) &&
             ((request.find(HTTP_CONTENT_LENGTH) != string::npos) ||
              (request.find(get_lower_string(HTTP_CONTENT_LENGTH)) != string::npos)))
    {
...
void CTcpTracerouteHandler::handle_preflight_request(
    string rAccessMethod, string rAccessHeaders, string rOrigin, string rConnection)
{
    string response = "";
...
string CTcpTracerouteHandler::get_value_from_string(string message, string key)
{
    ...
    if ((message.find(needle)) != string::npos)
    {
        ...
        if ((nPos1 = message.find(" ", nPosFirst)) != string::npos)
        {
            ...
            for (string::iterator it = message.begin() + nPos1 + 1;
                 it != message.begin() + nPos2; ++it)
            {
                value += *it;
            }
        }
        ...
        while (1)
        {
            ...
            if (mResponse != -1)
            {
                ...
                case AF_INET6:
                {
                    ...
                    inet_ntop(AF_INET6, &sockinfo_in6->sin6_addr.s6_addr, ip_address,
                              sizeof(ip_address));
                    break;
                }
                }
                ...
                else if (icmp_packet->icmp_type == ICMP_DEST_UNREACH &&
                         icmp_packet->icmp_code == ICMP_PKT_FILTERED)
                {
                    mRoute[ttl] = ip_address;
                    ...
                    exit = true;
                         ...
                         icmp_packet->icmp_code == ICMP_PORT_UNREACH)
                {
                    mRoute[ttl] = ip_address;
